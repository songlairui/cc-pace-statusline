...
（实际终端里每个条都坐在一层暗灰背景上，所以抖动到 0 的格子仍然作为灰槽可见，不会消失。）
...
每一格从五级密度里挑一个——`' ', ░, ▒, ▓, █`——用 **1D Floyd–Steinberg 误差扩散** 决定。**关键的是 levels 对齐了每个字形的"感知亮度"**（`░` 视觉上就是约 25% 而不是 33%），所以条的平均密度真正对应于百分比，而不是被低位字符的高视觉权重整体抬亮。不同百分比纹理区分度很高：
...
（方括号只是 README 里的文档装饰——实际终端里每条都套着一层暗灰背景，空格也会作为可见的灰槽呈现。）

附带好处：百分比每个点的小变化都会让纹理明显抖动，而朴素的二值条要等下一个 20% 阶跃才会动。
...
| --- | --- | --- | --- | --- |
| `ctx` | 暗灰底上抖动 `' '░▒▓█` | < 60% | 60–84% | ≥ 85% |
| `5h` | 暗灰底上抖动 `' '░▒▓█` | < 50% | 50–79% | ≥ 80% |
| `7d` | 暗灰底上抖动 `' '░▒▓█` | < 50% | 50–79% | ≥ 80% |
| `!200k` | — | — | — | 红 |

配速标记的颜色独立于条调色板：落后 = 亮绿，领先 = 亮品红。这样无论条本身是绿/黄/红、底色是灰，标记都仍然可见。
